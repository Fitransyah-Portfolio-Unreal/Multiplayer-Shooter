	...
	if ((SessionResults.Num() <= 0) || !bWasSuccesful)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				2.f,
				FColor::Red,
				FString::Printf(TEXT("There is no session available at the moment"))
			);
		}
	}
}
	...
	switch (Result)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				2.f,
				FColor::Red,
				FString::Printf(TEXT("Unknown error for trying join session"))
			);
		}
		break;
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				2.f,
				FColor::Red,
				FString::Printf(TEXT("Session you're looking for is not exist"))
			);
		}
		break;
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				2.f,
				FColor::Red,
				FString::Printf(TEXT("Could not retreive your session adress"))
			);
		}
		break;
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				2.f,
				FColor::Red,
				FString::Printf(TEXT("Session you're trying to join already full"))
			);
		}
		break;
	}
	...
	if (bWasSuccesful)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				3.f,
				FColor::Cyan,
				FString::Printf(TEXT("Session is destroyed"))
			);
		}
	}
	...
	if (bWasSuccesful)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				3.f,
				FColor::Cyan,
				FString::Printf(TEXT("Session is started"))
			);
		}
	}
}

void UMenu::DebugSubsystem()
{
	IOnlineSubsystem* Subsystem = IOnlineSubsystem::Get();
	if (Subsystem)
	{
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				3.f,
				FColor::Red,
				FString::Printf(TEXT("No subsystem found"))
			);
		}
	}
}
